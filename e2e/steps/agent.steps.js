...
Then('the agent receives feedback saying {string}', async ({ arev }, text) => {
...
Then('the agent receives {int} chat note(s)', async ({ arev }, count) => {
  const event = await arev.poll();
  expect(event.type).toBe('feedback');
...
When('the agent replies {string}', async ({ arev }, text) => {
